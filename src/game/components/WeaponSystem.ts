...
export class WeaponSystem extends Component {
  ...
  switchWeapon(type: string): boolean {
    ...
    if (!inventory) {
      console.warn(
        "WeaponSystem: No inventory found on entity, cannot switch weapon",
      );
      return false;
    }
  ...
  canFire(): boolean {
    ...
    if (!hasAmmo) {
      console.log("WeaponSystem.canFire: no ammo");
    }
    return hasAmmo;
  }
  ...
  fire(): boolean {
    ...
    window.dispatchEvent(
      new CustomEvent("weaponFired", {
        detail: { weapon: this.currentWeapon!.type },
      }),
    );
  ...
  private fireProjectile(direction: any) {
    ...
    if (hit?.hit && hit.pickedMesh && hit.pickedPoint) {
      setTimeout(
        ...
        ((hit.distance ?? 0) / (this.currentWeapon!.projectileSpeed || 30)) *
          1000,
      );
    }
  }
  ...
  private playEffect(position: any) {
    ...
    if (level === "megablast") {
      this.effects.playMegaBlast(position);
    } else if (level === "kaboom") {
      this.effects.playKaBoom(position);
    } else if (level === "bang") {
      this.effects.playBang(position);
    } else {
      this.effects.playPop(position);
    }
  }

...
export class StatusBar {
  ...
  responder(ev: DEvent): boolean {
    ...
    if (ev.type === EvType.KeyUp && (ev.data2 & 0xffff0000) === AM_MSGHEADER) {
      switch (ev.data2) {
        case AM_MSGENTERED:
      ...
      if (!this.game.netGame && this.game.gameSkill !== Skill.Nightmare) {
        if (this.cheat.checkCheat(godCheat, ev.data2)) {
          ...
          if (
            ...
            const levelNum =
              (buf[0].charCodeAt(0) - "0".charCodeAt(0)) * 10 +
            ...
            const levelNum =
              (buf[0].charCodeAt(0) - "1".charCodeAt(0)) * 9 +
  ...
  private calcPainOffset(): number {
    ...
    if (health !== this.oldHealthCalc) {
      this.lastCalc =
        FACE_STRIDE * ((((100 - health) * NUM_PAIN_FACES) / 101) >> 0);
  ...
  private updateFaceWidget(): void {
    ...
    if (this.priority < 8) {
      if (pl.damageCount && pl.attacker && pl.mo && pl.attacker !== pl.mo) {
        // being attacked
        this.priority = 7;
    ...
    if (this.priority < 5) {
      // invulnerability
      if (pl.cheats & PCheat.GodMode || pl.powers[PowerType.Invulnerability]) {
        this.priority = 4;
  ...
  private _doPaletteStuff(): void {
    ...
    if (cnt) {
      ...
      this.player.powers[PowerType.Ironfeet] > 4 * 32 ||
    ...
    if (
      ...
      palette >= START_RED_PALS &&
      palette < START_RED_PALS + NUM_RED_PALS
    ) {
      palette = RADIATION_PAL;
    }
  ...
  // 1108
  drawer(): void {
    // Candy HUD is rendered via React component, not here
  }

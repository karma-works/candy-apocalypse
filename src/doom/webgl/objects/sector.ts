...
export class Sector extends Group {
  ...
  constructor(
    ...
    const frontLines = lines.filter(
      ({ frontSector }) => frontSector === sector,
    );
  ...
  private updateTextureMap(lightLevel: number): void {
    ...
    const floorTex = this.textures.getFlatTexture(this.sector.floorPic);
    const floorIsSvg = floorTex.userData?.isSvg;
    if (floorIsSvg) {
      this.floor.material.defines = this.floor.material.defines || {};
      this.floor.material.defines["IS_SVG"] = "";
      this.floor.material.needsUpdate = true;
    }
    this.floor.material.map = floorTex;

    if (isSkyMesh(this.ceiling)) {
      this.ceiling.material.envMap = this.textures.getSkyTexture(
        this.sky!.texture,
      );
      ...
      const ceilTex = this.textures.getFlatTexture(this.sector.ceilingPic);
      const ceilIsSvg = ceilTex.userData?.isSvg;
      if (ceilIsSvg) {
        this.ceiling.material.defines = this.ceiling.material.defines || {};
        this.ceiling.material.defines["IS_SVG"] = "";
        this.ceiling.material.needsUpdate = true;
      }
      this.ceiling.material.map = ceilTex;
    }
  }
}

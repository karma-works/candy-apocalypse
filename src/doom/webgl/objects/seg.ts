...
export class Seg extends Group {
  ...
  constructor(
    ...
    if (seg.backSector) {
      if (
        seg.frontSector.ceilingPic === sky?.flatNum &&
  ...
  private createMesh(sky = false): SegMesh | SkyMesh {
    ...
    const x1 = v1.x / FRACUNIT,
      y1 = v1.y / FRACUNIT;
    const x2 = v2.x / FRACUNIT,
      y2 = v2.y / FRACUNIT;
    const dx = x1 - x2,
      dy = y1 - y2;
  ...
  private getMidOffset(
    ...
    if (lineDef.flags & MapLineFlag.DontPegBottom) {
      ...
      return (
        frontSector.floorHeight + textureHeight - frontSector.ceilingHeight
      );
    } else {
  ...
  private getTopOffset(
    ...
    if (lineDef.flags & MapLineFlag.DontPegTop) {
      ...
      if (
        frontSector.ceilingPic === this.sky?.flatNum &&
      ...
      // bottom of texture
      return (
        backSector.ceilingHeight + textureHeight - frontSector.ceilingHeight
      );
    }
  }
  ...
  private updateWallUvs(): void {
    ...
    if (!backSector) {
      ...
      if (sideDef.midTexture) {
        topOffset = this.getMaskedMidOffset(
          frontSector,
          backSector,
          lineDef,
          sideDef.midTexture,
  ...
  private updateUvs(
    ...
    mesh.material.texOffset.set(leftOffset / FRACUNIT, topOffset / FRACUNIT);
  ...
  private updateTextureMap(
    ...
    const isSvg = texture.map.userData?.isSvg;
    if (isSvg) {
      mesh.material.defines = mesh.material.defines || {};
      mesh.material.defines["IS_SVG"] = "";

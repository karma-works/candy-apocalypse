...
export class Rendering extends LegacyRendering {
  private camera = new PerspectiveCamera(64, 320 / 200, 10, 4000);
  ...
  constructor(
    ...
    if (doom.gui) {
      ...
      this.cameraFovController = this.cameraGui
        .add(this.camera, "fov")
        .min(1)
        .max(180)
  ...
  protected setupFrame(player: Player): void {
    ...
    this.textures.paletteTexture.colorMap =
      this.fixedColorMap || this.data.colorMaps.c[0];
    ...
    this.camera.rotation.x = toRad(-this.game.localLookPitch);

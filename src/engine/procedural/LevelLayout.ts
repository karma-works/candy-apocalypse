...
const CELL = 8;      // units per grid cell
...
interface RoomDef {
  id: number;
  gx: number; gz: number;  // grid origin (cell units)
  gw: number; gd: number;  // grid size (cell units)
...
function deriveSeed(p: LevelParams): number {
  return ((p.length * 73856093) ^ (p.difficulty * 19349663) ^ (p.complexity * 83492791)) >>> 0;
}
...
function makeRoom(
  ...
  gx: number, gz: number,
  gw: number, gd: number,
  isStart: boolean, isExit: boolean, isBranch: boolean,
): RoomDef {
  const r: RoomDef = {
    id, gx, gz, gw, gd,
    connections: [],
    isStart, isExit, isBranch,
    wx: 0, wz: 0, ww: 0, wd: 0, cx: 0, cz: 0,
  };
  setWorldCoords(r);
  return r;
}

function nextGridPos(parent: RoomDef, dir: Dir, newW: number, newD: number): { gx: number; gz: number } {
  switch (dir) {
    case "E": return { gx: parent.gx + parent.gw, gz: parent.gz };
    case "W": return { gx: parent.gx - newW,       gz: parent.gz };
    case "S": return { gx: parent.gx,              gz: parent.gz + parent.gd };
    case "N": return { gx: parent.gx,              gz: parent.gz - newD };
  }
}

// Which side of `a` faces `b` (rooms must be grid-adjacent)
function sharedSide(a: RoomDef, b: RoomDef): Dir {
  if (b.gx === a.gx + a.gw) return "E";
  if (b.gx + b.gw === a.gx) return "W";
  if (b.gz === a.gz + a.gd) return "S";
  return "N";
}
...
function splitSpan(start: number, end: number, gapCenters: number[]): Array<[number, number]> {
  ...
  for (const c of sorted) {
    const gs = c - DOOR_W / 2;
    const ge = c + DOOR_W / 2;
    if (gs > pos) segs.push([pos, Math.min(gs, end)]);
    pos = Math.max(pos, ge);
  }
  if (pos < end) segs.push([pos, end]);
...
export function generateLevel(params: LevelParams): GeneratedLevel {
  ...
  function tryPlaceAdjacentTo(parent: RoomDef, isExit: boolean, isBranch: boolean): RoomDef | null {
  ...
  for (let i = 1; i < critPathLen; i++) {
    ...
    const room = tryPlaceAdjacentTo(last, isExit, false);
    if (!room) break;
    last = room;
  }
  ...
  // ── Floors ────────────────────────────────────────────────────────────────
  const floors: FloorDef[] = rooms.map(r => ({ cx: r.cx, cz: r.cz, w: r.ww, d: r.wd }));
  ...
  for (const room of rooms) {
    ...
    for (const [s, e] of splitSpan(room.wx, room.wx + room.ww, doorways.S))
      walls.push({ cx: (s + e) / 2, cz: room.wz + room.wd, len: e - s, axis: "x" });
    ...
    for (const [s, e] of splitSpan(room.wz, room.wz + room.wd, doorways.E))
      walls.push({ cx: room.wx + room.ww, cz: (s + e) / 2, len: e - s, axis: "z" });
  }

  // ── Spawns ────────────────────────────────────────────────────────────────
  const spawns: SpawnPoint[] = [];

  function randomPosInRoom(r: RoomDef, margin: number): { x: number; z: number } {
  ...
  spawns.push({ type: "pickup-exit", position: [exitRoom.cx, 0.5, exitRoom.cz] });

  // Enemies
  function pickEnemyType(): string {
    if (params.difficulty <= 3) return "enemy-demon";
    if (params.difficulty <= 6) return rng.pick(["enemy-demon", "enemy-imp"]);
    return rng.pick(["enemy-demon", "enemy-imp", "enemy-imp", "enemy-cacodemon"]);
  }
  ...
  const pickupCount = Math.max(1, Math.round((11 - params.difficulty) + params.complexity * 0.3));
  const branchRooms = rooms.filter(r => r.isBranch);
  const otherRooms  = rooms.filter(r => !r.isStart && !r.isExit && !r.isBranch);
  const pickupRooms = [...branchRooms, ...otherRooms];
  const pickupTypes = ["pickup-health", "pickup-health", "pickup-ammo-pistol", "pickup-ammo-shotgun"];
  for (let i = 0; i < pickupCount; i++) {
    if (pickupRooms.length === 0) break;
